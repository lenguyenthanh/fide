...
object HistoryCrawler:
  ...
  def apply(client: Client[IO], config: CrawlConfig)(using Logger[IO]): HistoryCrawler = new:
    ...
    /** Merge players from all 3 categories by player ID. Each category file contains all fields but the
      * "primary" rating for that category is most reliable. We take the union of all players and merge
      * non-None fields, preferring the category-specific value for its own rating type.
      */
    ...
    private def merge(existing: NewPlayer, incoming: NewPlayer, category: String): NewPlayer =
      existing.copy(
        ...
        standard =
          if category == "standard" then incoming.standard.orElse(existing.standard)
          else existing.standard.orElse(incoming.standard),
        standardK =
          if category == "standard" then incoming.standardK.orElse(existing.standardK)
          else existing.standardK.orElse(incoming.standardK),
        rapid =
          if category == "rapid" then incoming.rapid.orElse(existing.rapid)
          else existing.rapid.orElse(incoming.rapid),
        rapidK =
          if category == "rapid" then incoming.rapidK.orElse(existing.rapidK)
          else existing.rapidK.orElse(incoming.rapidK),
        blitz =
          if category == "blitz" then incoming.blitz.orElse(existing.blitz)
          else existing.blitz.orElse(incoming.blitz),
        blitzK =
          if category == "blitz" then incoming.blitzK.orElse(existing.blitzK)
