...
/** Lazy-loaded, in-memory cache of PlayerId -> hash (Long).
  * Empty at boot. Populated on first access from DB.
...
object HashCache:
  ...
  def apply(loadFromDb: IO[Map[PlayerId, Long]]): IO[HashCache] =
    AtomicCell[IO].of(Option.empty[Map[PlayerId, Long]]).map: cell =>
      new HashCache:
        def get: IO[Map[PlayerId, Long]] =
          cell.evalModify:
