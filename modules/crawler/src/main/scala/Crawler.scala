...
object Crawler:
  ...
  def apply(db: Db, client: Client[IO])(using Logger[IO]): Crawler = new:
    def crawl: IO[Unit] =
      IO.realTimeInstant.flatMap(now => info"Start crawling at $now")
        *> Crawler.crawl(db, client).handleErrorWith(e => error"Error while crawling: $e")
  ...
  private def parseLine(line: String): IO[Option[(NewPlayer, Option[NewFederation])]] =
    ...
    parse(line)
      .pure[IO]
      .handleErrorWith(e => IO.println(s"Error while parsing line: $line, error: $e").as(None))
